  ...
  <Container>
    <div className="flex items-center justify-between py-md px-sm">
      <Link href="/" className="flex items-center gap-xs">
        <CldImage src="Kuma Tako Ranch/Logo/Logo" alt="Kuma Tako Ranch" width={36} height={36}/>
        <span className="text-h3 font-bold">Kuma Tako Ranch</span>
      </Link>
    <div className="hidden min-[900px]:flex items-center gap-lg">
      {navLinks.map((link) => (<Link key={link.href} href={link.href} className="text-small">{link.label}</Link>))}
    </div>
