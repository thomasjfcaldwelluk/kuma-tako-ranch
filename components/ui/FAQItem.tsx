  ...
  return (
    <div className={`border-b border-neutral-soft py-md px-md rounded-sm mt-xs ${variantStyles[variant]}`}>
      <button onClick={() => setIsOpen(!isOpen)} className="w-full flex items-center justify-between text-left">
        ...
        <Icon name="chevron-down" size="sm"className={`text-text-secondary transition-transform ${isOpen ? "rotate-180" : ""}`}/>
      </button>
      {isOpen && (<p className="text-small text-text-secondary mt-sm">{answer}</p>)}
    </div>
  );
}
